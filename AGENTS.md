...
- A `proper-lockfile` lock is a **directory**, not a file: it is acquired with `mkdir` and released with `rmdir`, and `proper-lockfile` does not retry either. On Windows — and far more often on virtual/synced drives like Google Drive File Stream (`G:\Shared drives\...`) — the release intermittently fails with `EBUSY`/`EPERM` because the sync agent, indexer, or antivirus still holds a handle. The failed release leaves the `.lock` directory on disk, so every later writer of that file stalls for the full `stale` window (10s) before it may steal the abandoned lock. The extension point is `LockOptions.fs` (typed `any`, defaults to `graceful-fs`): `src/fs/lockfile-fs.ts` supplies a long-lived adapter whose `rmdir`/`rmdirSync` route through `node:fs` `rm`/`rmSync` with `recursive`+`force`+`maxRetries`/`retryDelay`, which is Node's own EBUSY/EPERM backoff. Keep that adapter a module-level singleton — `proper-lockfile`'s mtime-precision probe caches on the object's identity, so a per-lock object re-probes (extra `utimes` + `stat`) on every acquisition.
